...
TmEcode ReceivePfringPluginThreadInit(ThreadVars *, const void *, void **);
void ReceivePfringPluginThreadExitStats(ThreadVars *, void *);
TmEcode ReceivePfringPluginThreadDeinit(ThreadVars *, void *);

TmEcode DecodePfringPluginThreadInit(ThreadVars *, const void *, void **);
TmEcode DecodePfringPlugin(ThreadVars *, Packet *, void *);
TmEcode DecodePfringPluginThreadDeinit(ThreadVars *tv, void *data);
 ...
 */
struct PfringPluginThreadVars_
{
...
{
    tmm_modules[slot].name = "ReceivePfringPlugin";
    tmm_modules[slot].ThreadInit = ReceivePfringPluginThreadInit;
    tmm_modules[slot].Func = NULL;
    tmm_modules[slot].PktAcqLoop = ReceivePfringPluginLoop;
    tmm_modules[slot].PktAcqBreakLoop = PfringPluginBreakLoop;
    tmm_modules[slot].ThreadExitPrintStats = ReceivePfringPluginThreadExitStats;
    tmm_modules[slot].ThreadDeinit = ReceivePfringPluginThreadDeinit;
    tmm_modules[slot].RegisterTests = NULL;
...
{
    tmm_modules[slot].name = "DecodePfringPlugin";
    tmm_modules[slot].ThreadInit = DecodePfringPluginThreadInit;
    tmm_modules[slot].Func = DecodePfringPlugin;
    tmm_modules[slot].ThreadExitPrintStats = NULL;
    tmm_modules[slot].ThreadDeinit = DecodePfringPluginThreadDeinit;
    tmm_modules[slot].RegisterTests = NULL;
    tmm_modules[slot].flags = TM_FLAG_DECODE_TM;
}

static inline void PfringDumpCounters(PfringPluginThreadVars *ptv)
{
 ...
 *
 * \param user pointer to PfringPluginThreadVars
...
{
    PfringPluginThreadVars *ptv = (PfringPluginThreadVars *)user;
...
static int PfringBypassCallback(Packet *p)
{
    ...
    PfringPluginPacketVars *vars = (PfringPluginPacketVars *)&p->plugin_v;
 ...
 * \param data pointer that gets cast into PfringPluginThreadVars for ptv
...
TmEcode ReceivePfringPluginLoop(ThreadVars *tv, void *data, void *slot)
{
    ...
    PfringPluginThreadVars *ptv = (PfringPluginThreadVars *)data;
    Packet *p = NULL;
 ...
 * \param data pointer gets populated with PfringPluginThreadVars
 * \todo add a config option for setting cluster id
 * \todo Create a general pfring setup function.
 * \retval TM_ECODE_OK on success
 * \retval TM_ECODE_FAILED on error
 */
TmEcode ReceivePfringPluginThreadInit(ThreadVars *tv, const void *initdata, void **data)
{
    int rc;
    u_int32_t version = 0;
    PfringPluginIfaceConfig *pfconf = (PfringPluginIfaceConfig *) initdata;
    unsigned int opflag;
    ...
    PfringPluginThreadVars *ptv = SCMalloc(sizeof(PfringPluginThreadVars));
    if (unlikely(ptv == NULL)) {
        pfconf->DerefFunc(pfconf);
        return TM_ECODE_FAILED;
    }
    memset(ptv, 0, sizeof(PfringPluginThreadVars));
 ...
 * \param data pointer that gets cast into PfringPluginThreadVars for ptv
 */
void ReceivePfringPluginThreadExitStats(ThreadVars *tv, void *data)
{
    PfringPluginThreadVars *ptv = (PfringPluginThreadVars *)data;
 ...
 * \param tv pointer to ThreadVars
 * \param data pointer that gets cast into PfringPluginThreadVars for ptvi
 * \retval TM_ECODE_OK is always returned
 */
TmEcode ReceivePfringPluginThreadDeinit(ThreadVars *tv, void *data)
{
    SCLogNotice("...");
    PfringPluginThreadVars *ptv = (PfringPluginThreadVars *)data;
    if (ptv->interface)
 ...
 * \retval TM_ECODE_OK is always returned
 */
TmEcode DecodePfringPlugin(ThreadVars *tv, Packet *p, void *data)
{
 ...
 */
TmEcode DecodePfringPluginThreadInit(ThreadVars *tv, const void *initdata, void **data)
{
